...
var multitonMySqlDBClient = make(map[interface{}]*MySqlDBClient)
var onceMySqlDBClient = make(map[interface{}]*sync.Once)
...
func GetMySqlDBClient(configurationHelper *helper.ConfigurationHelper) *MySqlDBClient {
	var locker sync.Mutex
	locker.Lock()
    defer locker.Unlock()
	if onceMySqlDBClient[configurationHelper] == nil {
		onceMySqlDBClient[configurationHelper] = &sync.Once{}
	}
	onceMySqlDBClient[configurationHelper].Do(func() {
		securityHelper := helper.GetSecurityHelper()
		multitonMySqlDBClient[configurationHelper] = &MySqlDBClient{
	...
	return multitonMySqlDBClient[configurationHelper]
}

func (pointer MySqlDBClient) GetDriver() string {
	return pointer.driver
}

func (pointer MySqlDBClient) GetHost() string {
	return pointer.host
}

func (pointer MySqlDBClient) GetPort() string {
	return pointer.port
}

func (pointer MySqlDBClient) GetDbName() string {
	return pointer.dbName
}

func (pointer MySqlDBClient) GetUser() string {
	return pointer.user
}

func (pointer MySqlDBClient) GetPassword() string {
	return pointer.password
}

func (pointer MySqlDBClient) GetCharset() string {
	return pointer.charset
}

func (pointer MySqlDBClient) DeployDatabase() {
	if pointer.deploy {
		...
		deployByFolder := func(folder string) error {
			...
			basepath = filepath.Clean(filepath.Join(basepath, ".."))
			basepath = basepath + "\\database\\mysql\\" + folder
